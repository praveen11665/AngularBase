<?php
if (!defined('BASEPATH')) exit('No direct script access allowed');

class Modules extends MY_Controller 
{
  public function __construct()
  {
    parent::__construct();
        
    // Load language
    $this->lang->load("setting","english");
    // Load Form and Form Validation
    $this->load->helper('form');
    $this->load->library('form_validation');
    // Check the user is loggedin or not
    $this->is_logged_in();
  }

  //To be check the acl condition and to be allow load form 
  public function add()
  {
    //Acl Permission To Load Form
    if( $this->acl_permits('setting.module_add') )
    {
        $this->loadForm();
    }
    else
    {
      //Unauthorized User Message
      $viewData = '';
      $data     = array(
                        'title'     =>  $this->lang->line('unauth_page_title'),
                        'content'   =>  $this->load->view('unauthorized',$viewData,TRUE)
                        );
      $this->load->view('base/error_template', $data);        
    }
  }

  //To be load the form with array values
  public function loadForm($formData = array())
  {
    $formData['ActionUrl']     = 'modules/Modules/formSubmit';   

    $viewData = array(
                        'form_title'        => $this->lang->line('module_form_title'),
                        'list_title'        => $this->lang->line('moduleoperations_list_title'),
                        'form_view'         => $this->load->view('modules/modules_form', $formData, TRUE)
                      );

    $tmpl = array ('table_open'  => '<table id="dataTableId" cellpadding="2" cellspacing="1" class="table table-striped">' );
    $this->table->set_template($tmpl); 
    $this->table->set_heading(lang('label_module_name'), lang('label_module_description'), lang('label_action'));

    $viewData['dataTableUrl']  = 'modules/Modules/datatable';
    $viewData['message']       = $this->session->flashdata('msg');
    $viewData['alertType']     = $this->session->flashdata('alertType');

    $data = array(
                    'title'     =>  $this->dbvars->app_name.' - '.$this->lang->line('module_form_title'),
                    'content'   =>  $this->load->view('base/form_template', $viewData,TRUE)
                  );

    $this->load->view($this->dbvars->app_template, $data);
  }

  //To be Submit the form with POST values. The form either Inset or Edit with the codtion
  public function formSubmit($viewData='')
  {
    if(!empty($_POST))
    {
      //Validation Rules
      $this->form_validation->set_rules('category_code', $this->lang->line('label_module'), 'required|alpha');          
      $this->form_validation->set_rules('category_desc', $this->lang->line('label_module_description'), 'required|trim');      
      
      if($this->form_validation->run() == TRUE)
      {
        if($this->input->post('category_id') == "")
        {
          //Insert
          $data       = array(                                              
                              'category_code'   => $this->input->post('category_code'),
                              'category_desc'   => $this->input->post('category_desc'),
                              );
          $result     = $this->mcommon->common_insert('acl_categories', $data);

          if($result)
          {
            //Success Message After Insertion
            $this->session->set_flashdata('msg', 'Saved Successfully');
            $this->session->set_flashdata('alertType', 'success');
            redirect(base_url('modules/Modules/add'));
          }
        }
        else
        {
          //Update
          $data       = array(                                              
                              'category_code'   => $this->input->post('category_code'),
                              'category_desc'   => $this->input->post('category_desc'),
                              );
                     
          $where_array = array('category_id' => $this->input->post('category_id'));
          $result      = $this->mcommon->common_edit('acl_categories', $data, $where_array);
   
          if($result)
          {
            //Success Message After Update
            $this->session->set_flashdata('msg', 'Updated Successfully');
            $this->session->set_flashdata('alertType', 'success');
            redirect(base_url('modules/Modules/add'));
          }
          else
          {
            //Message while Submitting Form Without Any Update
            $this->session->set_flashdata('msg', 'No Data Has Been Changed');
            $this->session->set_flashdata('alertType', 'danger');
            redirect(base_url('modules/Modules/add'));
          }  
        }
      }
    }

    $this->loadForm();
  } 

  //To be Edit the Data
  public function edit($category_id='')
  {
    //Acl Permission For Edit
    if( $this->acl_permits('setting.module_edit') )
    {
      $constraint_array           = array('category_id' => $category_id);             
      $formData['tabledata']      = $this->mcommon->records_all('acl_categories', $constraint_array);
      $this->loadForm($formData);
    }
    else
    {
      //Unauthorized User Message
      $viewData = '';
      $data     = array(
                        'title'     =>  $this->lang->line('unauth_page_title'),
                        'content'   =>  $this->load->view('unauthorized',$viewData,TRUE)
                        );
      $this->load->view('base/error_template', $data);        
    }
  }

  //To be Delete the Data
  public function delete($category_id='')
  {
    //Acl Permission For Delete
    if( $this->acl_permits('setting.module_delete') )
    {
      $where_array = array('category_id' => $category_id);
      $result      = $this->mcommon->common_delete('acl_categories',$where_array);
      
      if($result)
      {
        $this->session->set_flashdata('msg', 'Deleted Successfully');
        $this->session->set_flashdata('alertType', 'success');
        redirect(base_url('setting/modules/Modules/add'));
      }
    }
    else
    {
      //Unauthorized User Message
      $viewData = '';
      $data     = array(
                        'title'     =>  $this->lang->line('unauth_page_title'),
                        'content'   =>  $this->load->view('unauthorized',$viewData,TRUE)
                        );
      $this->load->view('base/error_template', $data);        
    }
  }
  
  //Take records and view to datatable
  public function datatable()
  {
    $this->datatables ->select('c.category_code, c.category_desc, c.category_id')
    ->from('acl_categories as c');                                       
    $this->datatables ->edit_column('c.category_id', '$1', 'get_buttons(c.category_id, "modules/modules/")');

    echo $this->datatables->generate();
  }
}
