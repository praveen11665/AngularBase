...
class Buyer_list extends MY_Controller
{
  ...
  /*To be check the acl condition and to be allow loadform*/
  public function add()
  {
    //Acl permission to load
    if($this->acl_permits('vendor.approved_buyer_list') )
    {
      $this->loadForm();
    }
    else
    {
      //Un Authorized user Message
      $viewData = '';
      $data    = array(
  ...
  public function loadForm($viewData=array())
  {
    ...
    $searchData['ActionUrl']    =  'buyer_list/add';
    $searchData['filterArr']    =  array('from_date', 'to_date', 'country', 'state', 'status'); 
    
    $viewData = array(
                      'form_title'        => $this->lang->line('title_av_buyer_list'),
                      'list_title'        => $this->lang->line('title_av_buyer_list'),
    ...
    $viewData['dataTableUrl']     = 'Buyer_list/datatable';
    
    $data = array (
                    'title'     =>  $this->dbvars->app_name.' - '.$this->lang->line('title_av_buyer_list'),
  ...
  function datatable()
  {
    ...
    $whereArray = array('u.auth_level' => '5', 'up.profile_status' => '1', 'up.document_status' => '1');
    ...
    $this->datatables->edit_column('u.user_id', '$1','get_angular_view(u.user_id, "Buyer_list/angularViewForm", 1)');
